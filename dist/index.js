...
if (existsSync(configFile))
    bind(configFile, encryptedFile), process.exit(1);
if (!existsSync(encryptedFile))
    log(`Neither ${configFile} nor ${encryptedFile} found.`, 'error'), process.exit(1);
if (!process.env.IV?.length)
    process.env.IV = question('Enter Initialization Vector (IV) : ');
if (!process.env.KEY?.length)
    process.env.KEY = question('Enter Decryption key (Key) : ');
...
const clusterManager = new ClusterManager(file, {
    ...
    token: 'MTM0NzYxNDI0NTAzNDAwMDQ2Nw.Gzv7yu.mcdqC1iYFFfv25v3eGzM_GK767WbfojdK2vuNk',
});
